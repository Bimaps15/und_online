...
async function postJson(url: string, body: unknown): Promise<ApiResult> {
  ...
  try {
    data = JSON.parse(responseText) as ApiResult
  } catch {
    throw new Error('Layanan konfirmasi sedang tidak tersedia. Silakan coba beberapa saat lagi.')
  }

  if (!response.ok || !data.success) {
    throw new Error(data.message || 'Konfirmasi belum dapat dikirim.')
  }
...
export function RSVP() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const formElement = event.currentTarget; setState('loading'); const form = new FormData(formElement); try { const result = await postJson('/api/rsvp', { name: form.get('name'), attendance: form.get('attendance'), guestCount: Number(form.get('guestCount')), message: form.get('message'), turnstileToken: token }); setFeedback(result.message); setState('success'); formElement.reset() } catch (error) { setFeedback(error instanceof Error ? error.message : 'Konfirmasi belum dapat dikirim.'); setState('error') } }
...
export function Wishes() {
  ...
  useEffect(() => { void load() }, [])
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const formElement = event.currentTarget; setState('loading'); const form = new FormData(formElement); try { const result = await postJson('/api/wishes', { name: form.get('name'), message: form.get('message'), turnstileToken: token }); setFeedback(result.message); setState('success'); formElement.reset(); load() } catch (error) { setFeedback(error instanceof Error ? error.message : 'Ucapan belum dapat dikirim.'); setState('error') } }
